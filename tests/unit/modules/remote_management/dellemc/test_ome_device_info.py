...
import pytest
from ansible_collections.ansible.misc.tests.unit.modules.utils import set_module_args, exit_json, \
    fail_json, AnsibleFailJson, AnsibleExitJson
from ansible.module_utils import basic
from ansible_collections.ansible.misc.plugins.modules import ome_device_info
...
class TestOmeDeviceInfo(object):
    ...
    def connection_mock(self, mocker):
        connection_class_mock = mocker.patch('ansible_collections.ansible.misc.plugins.modules.ome_device_info.RestOME')
        return connection_class_mock.return_value

    @pytest.fixture
    def response_mock(self, mocker):
        response_class_mock = mocker.patch('ansible_collections.ansible.misc.plugins.module_utils.remote_management.dellemc.ome.OpenURLResponse')
        return response_class_mock

    @pytest.fixture
    def validate_inputs_mock(self, mocker):
        response_class_mock = mocker.patch('ansible_collections.ansible.misc.plugins.modules.ome_device_info._validate_inputs')
        response_class_mock.return_value = None

    @pytest.fixture
    def get_device_identifier_map_mock(self, mocker):
        response_class_mock = mocker.patch('ansible_collections.ansible.misc.plugins.modules.ome_device_info._get_device_identifier_map')
    ...
    def get_resource_parameters_mock(self, mocker):
        response_class_mock = mocker.patch('ansible_collections.ansible.misc.plugins.modules.ome_device_info._get_resource_parameters')
        return response_class_mock
    ...
    def test_validate_inputs(self, fact_subset, mutually_exclusive_call, mocker):
        ...
        check_mutually_inclusive_arguments_mock = mocker.patch(
            'ansible_collections.ansible.misc.plugins.modules.ome_device_info._check_mutually_inclusive_arguments')
    ...
    def test_get_device_identifier_map(self, module_params, connection_mock, mocker):
        get_device_id_from_service_tags_mock = mocker.patch('ansible_collections.ansible.misc.plugins.modules.ome_device_info._get_device_id_from_service_tags')
